...
def get_args_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default="geom",
        choices=["geom", "feat", "xyz", "both"],
        help="Which uncertainty path to train (geometry-depth UQ, feature UQ, 3D xyz UQ, or both)",
    )
...
def train(args):
    ...
    def _unfreeze_xyz_uq(model_to_freeze):
        """Unfreeze the new 3D UQ head on CatConv: head_xyz_evi."""
        unfrozen = False
        for head_attr in ("downstream_head1", "downstream_head2"):
            head = getattr(model_to_freeze, head_attr, None)
            if head is None:
                continue
            if hasattr(head, "head_xyz_evi"):
                for p in head.head_xyz_evi.parameters():
                    p.requires_grad = True
                unfrozen = True
        return unfrozen
    ...
    def freeze_for_uq(model_to_freeze, args):
        ...
        has_xyz_head = any(
            hasattr(getattr(model_to_freeze, h, None), "head_xyz_evi")
            for h in ("downstream_head1", "downstream_head2")
        )
    ...
    if getattr(args, "freeze_geom_for_uq", False):
        if misc.get_rank() == 0:
        ...
        if misc.get_rank() == 0:
            ...
            for n in trainable:
                print("  ", n)
            print("Only head_xyz_evi?", all("head_xyz_evi" in n for n in trainable))
